  ...
  <div>
    ...
    <ul>
      {anecdotes.map(anecdote =>
        <li key={anecdote.id} style={{display: "flex"}}>
          <Link to={`/anecdotes/${anecdote.id}`} >{anecdote.content}</Link>
          <p style={{margin: "0 10px"}}>has: {anecdote.votes}</p>
          <button
            onClick={() => addVote(anecdote.id)}
...
const CreateNew = (props) => {
  ...
  const handleSubmit = (e) => {
    ...
    props.addNew({
      ...
      conauthortent: author.forInput.value,
      info: info.forInput.value,
      votes: 0
    })
    history.push('/')
    props.createNotification(content.forInput.value)
  }
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <button type="button" onClick={reset}>create</button>
      </form>
    </div>
  )
}
...
const App = () => {
  ...
  return (
      <div>
        ...
        <Switch>
          <Route path="/" exact>
            <AnecdoteList anecdotes={anecdotes} addVote={vote} />
          </Route>
